...
function calcNote(note1, note2) {
  return Number((note1 + note2) / 2);
}

for (let student of students) {
  const gradeAverage = calcNote(student.note1, student.note2);
   const message = `O(A) aluno(a) ${student.name} teve uma média de:${gradeAverage}`

  if (gradeAverage < 7) {
    alert(`${message} \n${student.name} prepare-se para a recupração!`);
  } else { 
    alert(`${message} \nParabéns ${student.name} você passou no concurso com êxito`);
  }
}
